...
public class TopicPartitionProcessor<K, V> extends Thread {
    ...
    public TopicPartitionProcessor(PriorityBlockingQueue<DelayItem<K, V>> queue,
        ...
        if (delayItemHandler == null) {
            throw new IllegalArgumentException("DelayItemHandler cannot be null");
        }
        if (topicPartition == null) {
            throw new IllegalArgumentException("TopicPartition cannot be null");
        }
        // commitCallback 可以为 null，因为某些场景下可能不需要提交偏移量
    ...
    private void executeDelayItemSync(DelayItem<K, V> item) {
        ...
        // 记录处理开始
        log.debug("processing record topic={} partition={} offset={} dueAt={} now={}",
                topicPartition.topic(), topicPartition.partition(),
                record.offset(), item.getResumeAtTimestamp(), System.currentTimeMillis());

        try {
            log.trace("Calling delayItemHandler.process for record with key={}, offset={}",
                    record.key(), record.offset());

            delayItemHandler.process(item);

            // 记录处理完成
            log.trace("processed record topic={} partition={} offset={} committedNext={}",
                    topicPartition.topic(), topicPartition.partition(),
    ...
    private void executeDelayItemAsync(DelayItem<K, V> item) {
        ...
        // 记录处理开始
        log.debug("submitting async processing for record topic={} partition={} offset={} dueAt={} now={}",
                topicPartition.topic(), topicPartition.partition(),
        ...
        CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
            try {
                ...
                log.trace("completed async processing for record topic={} partition={} offset={}",
